...
class abrir_publicacionModel extends Model{
    ...
    function eliminarPostulacion($id_postulacion){
        ...
        try {
            ...
            $query->exec("INSERT INTO `NOTIFICACION` (`MOTIVO`,`FECHA_ALTA`,`ESTADO`,`URL`,`ID_USUARIO`) VALUES(4,'".$dataTime."',1,'abrir_publicacion?publicacion=".$datos['ID_PUBLICACION']."',".$datos['ID_USUARIO_POSTULADO'].");");
            $query->commit();
            return true;
        }catch(PDOException $exc) {
            $query->rollback();
    ...
    function getAdopcion($id_postulacion){
        ...
        try {
            ...
            if($datos['ID_ESTADO']==1){
                 ...
                 $query->exec("UPDATE `PUBLICACION` SET`ID_ESTADO`= 4 WHERE ID_PUBLICACION = ".$datos['ID_PUBLICACION']);
            }else{
                $query->exec("UPDATE `PUBLICACION` SET`ID_ESTADO`= 5 WHERE ID_PUBLICACION = ".$datos['ID_PUBLICACION']);
            ...
            $query->beginTransaction();  
            $postulados=$this->traerPostulados($datos['ID_PUBLICACION']);
            
            $query->exec("UPDATE `POSTULACION` SET `ESTADO_POSTULACION`= 2 WHERE `ID_PUBLICACION` = ".$datos['ID_PUBLICACION']." AND `ESTADO_POSTULACION`= 1; ");
            
            foreach ($postulados as $postulado){
            $query->exec("INSERT INTO `NOTIFICACION` (`MOTIVO`,`FECHA_ALTA`,`ESTADO`,`URL`,`ID_USUARIO`) VALUES(4,'".$dataTime."',1,'abrir_publicacion?publicacion=".$datos['ID_PUBLICACION']."',".$postulado['ID_POSTULADO'].");");
            }
    ...
    function traerDatos($id_postulacion){
        $query = $this->db->connect();
        $query= $this->db->connect()->prepare("SELECT `POSTULACION`.`ID_USUARIO_POSTULADO`, `POSTULACION`.`ID_PUBLICACION`, `P`.`ID_ESTADO` FROM  (`POSTULACION` 
        JOIN `PUBLICACION` `P` ON ((`P`.`ID_PUBLICACION` = `POSTULACION`.`ID_PUBLICACION`)))  WHERE ID_POSTULACION = ".$id_postulacion.";");
        $query->execute();
        return $query->fetch();
    }
    
    function traerPostulados($id_publicacion){
        $items=[];
        $query = $this->db->connect();
        $query= $this->db->connect()->prepare("SELECT `ID_USUARIO_POSTULADO`FROM `POSTULACION` WHERE `ID_PUBLICACION` = ".$id_publicacion." AND `ESTADO_POSTULACION`= 1;");
        $query->execute();
        while($row = $query->fetch()){
                $item=[
                    'ID_POSTULADO' => $row['ID_USUARIO_POSTULADO']
                ];
                array_push($items,$item);
            }
            return $items;
