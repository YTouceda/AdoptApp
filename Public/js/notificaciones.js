    ...
    $.ajax({
        ...
        success:  function (response){
            ...
            $(response["NOTIFICACIONES"]).each((index)=>{
                ...
                nuevaNoti +="<div id='notificacion"+index+"' id_notificacion='"+response["NOTIFICACIONES"][index].ID_NOTIFICACION+"' class='position-relative list-group-item list-group-item-action notification_hover div_notificacion_opc px-2'>"+
                ...
                nuevaNoti +=        "</div>"+
                                    ...
                                    "<div class='div_botones invisible'>"+
                                        "<button id='btn_eliminar_notificacion"+[index]+"' class='btn btn-link btn_notificacion px-1 mx-0 mx-xl-2'>Eliminar notificación</button>"+
                                        "<button id='btn_cambiar_estado"+[index]+"' class='btn btn-link btn_notificacion px-1 mx-0 mx-xl-2'>Marcar como ";
                if(response["NOTIFICACIONES"][index].ESTADO == "no leído"){
            ...
            if($(window).width() < 1000){
                $('div.div_botones').removeClass('invisible');
            }else{
                $('div.div_botones').addClass('invisible');
