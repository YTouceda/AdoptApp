...
class abrir_publicacion extends Controller{
    ...
    function Render(){
        ...
        $this->view->estado_post = $this->validarEstadoPostulacion();
        $this->view->tiene_post = $this->validarPostulacion();
    ...
    function validarEstadoPostulacion(){
        if(empty($this->view->postulaciones)){
            return true;
        }else{
            $resultado_post=true;
            foreach ($this->view->postulaciones as $postulacion){
                if($postulacion['USUARIO_POSTULADO']->getId()==$_SESSION['id'] && $postulacion['ESTADO_POSTULACION'] == 2){
                    $resultado_post=false;
                }
            }
            return $resultado_post;
    ...
    function eliminar_postulacion(){
        if(isset($_GET['id_postulacion'])){
            //die(var_dump($_GET['id_postulacion']));
